...
export const BottomConsole: React.FC = () => {
  ...
  if (!isConsoleOpen) {
    return (
      <button
        ...
        className="h-6 bg-[#090d14] border-t border-slate-800 px-3 flex items-center justify-between text-[11px] font-mono text-slate-400 hover:text-cyan-300 select-none z-30"
      >
  ...
  return (
    <div className="h-36 bg-[#090d14]/98 border-t border-slate-800 flex flex-col font-mono text-xs select-none z-30 shadow-2xl">
      {/* Console Tab Header */}
      <div className="flex items-center justify-between px-3 py-1 bg-slate-950 border-b border-slate-800/80">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className={`p-1 rounded text-slate-400 hover:text-slate-200 ${isPaused ? 'text-amber-400' : ''}`}
      ...
      {/* Log Output Area */}
      <div className="flex-1 overflow-y-auto p-2 font-mono text-[11px] space-y-1 bg-black/40">
        {logs.map((log) => (
          <div key={log.id} className="flex items-start space-x-2">
            <span className="text-slate-500 tabular-nums">{formatTimestamp(log.time)}</span>
            <span className="px-1 rounded bg-slate-900 border border-slate-800 text-[9px] text-cyan-400 font-bold">
              {log.topic}
            </span>
