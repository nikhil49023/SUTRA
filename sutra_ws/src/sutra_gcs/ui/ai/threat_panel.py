"""
Smart Horizon GCS — AI Threat Assessment & Matrix Panel
...
class ThreatPanel(QWidget):
    """
    Tactical threat matrix table displaying verified hazards, geofence breaches, and terrain obstructions.
    """
    ...
    def _init_ui(self) -> None:
        ...
        hdr = QLabel("TACTICAL THREAT MATRIX")
        hdr.setStyleSheet("color: #00f2fe; font-size: 11px; font-weight: 800; letter-spacing: 1px;")
        layout.addWidget(hdr)

        self.table = QTableWidget(0, 5, self)
        self.table.setHorizontalHeaderLabels(["THREAT LABEL", "SEVERITY", "DISTANCE", "SOURCE", "CONFIDENCE"])
        ...
        self.empty_lbl = QLabel("NO ACTIVE THREATS")
        self.empty_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.empty_lbl.setStyleSheet("color: #10b981; font-size: 10px; font-weight: bold; padding: 12px;")
        layout.addWidget(self.empty_lbl)

    def _on_state_updated(self, state: ApplicationState) -> None:
        threats = state.ai_state.threats
        if not threats:
        ...
        for row, t in enumerate(threats):
            self.table.setItem(row, 0, QTableWidgetItem(t.label))
            
            sev_item = QTableWidgetItem(t.severity.value)
            if t.severity.value in ("EMERGENCY", "CRITICAL"):
                sev_item.setForeground(Qt.GlobalColor.red)
            elif t.severity.value == "HIGH":
                sev_item.setForeground(Qt.GlobalColor.yellow)
            self.table.setItem(row, 1, sev_item)

            self.table.setItem(row, 2, QTableWidgetItem(f"{t.distance_m:.0f}m"))
            self.table.setItem(row, 3, QTableWidgetItem(t.source))
