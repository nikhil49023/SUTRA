...
class SutraByteTracker:
    ...
    def update(self, detections: Any) -> List[SutraByteTrackerTrack]:
        # Increment time_since_update for all active tracks
        for track in self.tracks:
            track.time_since_update += 1

        norm_dets: List[Tuple[Tuple[float, float, float, float], float, str]] = []
        for d in detections:
            if isinstance(d, dict):
                bbox = tuple(d.get("bbox", (0, 0, 0, 0)))
                score = float(d.get("confidence", d.get("score", 0.9)))
                label = str(d.get("label", "person"))
                norm_dets.append((bbox, score, label))
            elif isinstance(d, (list, tuple)) and len(d) >= 3:
                norm_dets.append((tuple(d[0]), float(d[1]), str(d[2])))

        unmatched_dets = list(range(len(norm_dets)))
        ...
        for track in self.tracks:
            ...
            for idx in unmatched_dets:
                bbox, score, label = norm_dets[idx]
            ...
            if best_iou >= self.iou_thresh and best_det_idx != -1:
                bbox, score, label = norm_dets[best_det_idx]
